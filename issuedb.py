...
_SQL_LOG = False
new_issue_sql = """create table if not exists {} (
  title     VARCHAR(500),
...
drop_issue_sql = "DROP TABLE IF EXISTS {};"

insert_issue_sql = """INSERT INTO {} (title,user,id,issue_num,comments,labels,state,created,updated,closed,body)
VALUES ('{}','{}',{},{},{},'{}','{}','{}','{}','{}','{}');"""

select_issue_sql = """SELECT title,user,id,issue_num,comments,labels,state,created,updated,closed,body from {};"""


def get_issue_header():
    return ["title", "requester", "id", "issue_num", "label", "comments", "state", "create day", "update day",
            "close day", "body"]


def name_tuple_formatter(header, data):
...
def insert_table(cursor, tb_name, data):
    ...
    exc_sql = insert_issue_sql.format(*tmp_data)
    if _SQL_LOG:
...
def create_table(cursor, tb_name):
    exc_sql = new_issue_sql.format(tb_name)
    if _SQL_LOG:
        logger.debug(exc_sql)
    cursor.execute(exc_sql)
...
def drop_table(cursor, tb_name):
    exc_sql = drop_issue_sql.format(tb_name)
    if _SQL_LOG:
        logger.debug(exc_sql)
    cursor.execute(exc_sql)
...
class ISSuedb:
    ...
    def db_new_issue_table(self, tb_name):
        create_table(self.cursor, tb_name)
        self._db_commit()

    def db_insert_issue_row(self, tb_name, data):
    ...
    def dump_csv(self, tb_name):
        with open(tb_name + '.csv', "w", encoding="utf_8_sig", newline='') as app_review_file:
            ...
            f_csv.writerow(get_issue_header())
            self.cursor.execute(select_issue_sql.format(tb_name))
            rows = self.cursor.fetchall()
...
if __name__ == '__main__':
    rdb = ISSuedb()
    rdb.db_new_issue_table("nextcloud$android")
